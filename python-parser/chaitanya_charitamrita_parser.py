"""
Chaitanya Charitamrita parser for vedabase.io
"""
import re
...
class ChaitanyaCharitamritaParser(BaseVedabaseParser):
    """Parser for Chaitanya Charitamrita from vedabase.io"""
    
    def __init__(self, config: dict = None):
        super().__init__('cc', config)
    
    async def parse_chapter(self, part: str, chapter_number: int) -> List[ParsedVerse]:
        """Parse a specific chapter of Chaitanya Charitamrita"""
        # For CC, we need to parse at verse level: cc/part/chapter/verse
        verses = []
        
        # Try to find verses by checking verse-level URLs
        verse_number = 1
        while verse_number <= 50:  # Reasonable limit per chapter
            url = f"{self.base_url}{part}/{chapter_number}/{verse_number}/"
            ...
            verse = self._extract_verse_from_html(soup, part, chapter_number, verse_number)
        ...
        self.logger.info(f"Parsed {len(verses)} verses from CC {part}.{chapter_number}")
        return verses
    
    async def parse_all_chapters(self) -> 'ParseResult':
        """Parse all chapters of Chaitanya Charitamrita (override base method)"""
        ...
        self.logger.info(f"Starting to parse {self.text_name} (3 parts)")
        
        try:
            # Parse all parts and chapters
            parts = ['adi', 'madhya', 'antya']
            part_chapters = {'adi': 17, 'madhya': 25, 'antya': 20}
            
            for part in parts:
                self.logger.info(f"Parsing {part.upper()}-lila")
                max_chapters = part_chapters[part]
                
                for chapter_num in range(1, max_chapters + 1):
                    try:
                        verses = await self.parse_chapter(part, chapter_num)
                        if verses:
                            result.verses.extend(verses)
                            result.successful_verses += len(verses)
                            self.logger.info(f"{part.upper()}-lila, Chapter {chapter_num}: {len(verses)} verses")
                        else:
                            # If no verses found, we've probably reached the end of this chapter
                            continue
                    except Exception as e:
                        error_msg = f"Error parsing {part}-lila, chapter {chapter_num}: {e}"
    ...
    def _extract_verse_from_html(self, soup: BeautifulSoup, part: str, chapter_number: int, verse_number: int) -> ParsedVerse:
        """Extract a single verse from CC HTML"""
        try:
            ...
            self.logger.error(f"Error extracting verse {part}.{chapter_number}.{verse_number}: {e}")
            return None
    
    def _extract_verses_from_html(self, soup: BeautifulSoup, chapter_number: int) -> List[ParsedVerse]:
        """Extract verses from Chaitanya Charitamrita HTML"""
        verses = []
